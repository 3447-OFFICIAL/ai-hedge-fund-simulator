...
class BacktestEngine:
    ...
    def run_backtest(
        self, historical_data: pd.DataFrame, weights: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        ...
        if isinstance(historical_data.columns, pd.MultiIndex):
            for ticker in weights.keys():
                if (
                    "Close" in historical_data.columns.levels[0]
                    and ticker in historical_data.columns.levels[1]
                ):
        ...
        drawdown = (
            strat.analyzers.drawdown.get_analysis().get("max", {}).get("drawdown", 0.0)
        )
